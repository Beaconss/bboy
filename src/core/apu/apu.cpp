...
uint8 APU::read(const Index index, uint8 waveRamIndex)
{
  ...
  case audioCtrl:
    return m_audioControl | 0x70 |
           (m_channel4.isEnabled() << 3 | m_channel3.isEnabled() << 2 | (m_channel2.isEnabled()) << 1 |
            (m_channel1.isEnabled()));
...
{
  return reg != ch1TimDuty && reg != ch2TimDuty && reg != ch3Tim && reg != ch4Tim && reg != audioCtrl && reg != waveRam;
}
...
void APU::mCycle()
{
  ...
  float rightSample{((ch1Sample * (m_audioPanning & ch1Right)) + (ch2Sample * ((m_audioPanning & ch2Right) >> 1)) +
                     (ch3Sample * ((m_audioPanning & ch3Right) >> 2)) + (ch4Sample * ((m_audioPanning & ch4Right) >> 3))) /
                    4.f};

  float leftSample{((ch1Sample * ((m_audioPanning & ch1Left) >> 4)) + (ch2Sample * ((m_audioPanning & ch2Left) >> 5)) +
                    (ch3Sample * ((m_audioPanning & ch3Left) >> 6)) + (ch4Sample * ((m_audioPanning & ch4Left) >> 7))) /
                   4.f};
