...
{
  if(constexpr uint8 disableTimerBit{0b0100'0000}; !(m_periodHighAndControl & disableTimerBit) || m_disableTimer == 0)
    return;
...
void channels::WaveChannel::setPushTimer()
{
  ...
  m_pushTimer = ((maxPeriod + 1) - (m_periodLow | ((m_periodHighAndControl & periodHighBits)
                                                   << 8))) /*this should be multiplied by 2 but it sounds better like this*/;
}
