...
void PPU::mCycle()
{
  ...
  switch(m_mode)
  {
  case oamScan: oamScanCycle(); break;
  case drawing: drawingCycle(); break;
  case hBlank:  hBlankCycle(); break;
  case vBlank:  vBlankCycle(); break;
  }
}
...
void PPU::write(const Index index, const uint8 value)
{
  ...
  case lcdc:
    ...
    {
      updateMode(hBlank);
      m_stat = (m_stat & 0b1111'1100) | m_mode; // manually update stat mode bits because if the ppu is
                                                // disabled they wont update
      m_ly = 0;
      m_cycleCounter = 20;
    }
...
{
  const bool statResult{m_statInterrupt.sources[StatInterrupt::hBLank] || m_statInterrupt.sources[StatInterrupt::vBlank] ||
...
void PPU::setStatModeSources()
{
  for(int i{0}; i < 3; ++i)
  {
    if(m_stat & (1 << (3 + i)) && i == m_mode)
      m_statInterrupt.sources[i] = true; // at bits 3-4-5 are stored the stat condition enable for mode
...
void PPU::oamScanCycle()
{
  ...
  if(m_cycleCounter == oamScanEndCycle || m_cycleCounter == 19 && m_ly == 0)
  {
    ...
    // left goes up because the left one has higher priority since its first in
    // oam memory
    std::sort(m_spriteBuffer.begin(), m_spriteBuffer.end(),
              [](Sprite left, Sprite right) { return left.xPosition >= right.xPosition; });
    m_spriteAddress = oamMemoryStart;
    updateMode(drawing);
  }
}
...
void PPU::tryAddSpriteToBuffer(const Sprite sprite)
{
  ...
  if(m_ly + 16 >= sprite.yPosition && m_ly + 16 < sprite.yPosition + (m_lcdc & tallSpriteMode ? 16 : 8))
  {
...
void PPU::hBlankCycle()
{
  ...
  if(m_ly == firstVBlankScanline) // if this next scanline is the first of
                                  // vBlank, vBlank for another 10 scanlines
  {
    m_vblankInterruptNextCycle = true;
    updateMode(vBlank);
    constexpr uint8 statOamSourceEnable{0b10'0000};
    if(m_stat & statOamSourceEnable)
      m_statInterrupt.sources[oamScan] = true; // re enable oam scan source if the corresponding stat bit is
                                               // active as updateMode(vBlank) cleared it
  }
}
